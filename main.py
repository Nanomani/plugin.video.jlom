...
def list_movies(movie_list):
    ...
    if ordered_by == "":
        ...
        xbmcplugin.addSortMethod(HANDLE, xbmcplugin.SORT_METHOD_VIDEO_RATING)
        xbmcplugin.addSortMethod(HANDLE, xbmcplugin.SORT_METHOD_VIDEO_RUNTIME)
    elif ordered_by == "rating":
        xbmcplugin.addSortMethod(HANDLE, xbmcplugin.SORT_METHOD_VIDEO_RATING)#default
        xbmcplugin.addSortMethod(HANDLE, xbmcplugin.SORT_METHOD_TITLE)
        xbmcplugin.addSortMethod(HANDLE, xbmcplugin.SORT_METHOD_VIDEO_YEAR)
        xbmcplugin.addSortMethod(HANDLE, xbmcplugin.SORT_METHOD_VIDEO_RUNTIME)
    elif ordered_by == "runtime":
        xbmcplugin.addSortMethod(HANDLE, xbmcplugin.SORT_METHOD_VIDEO_RUNTIME)#default
        xbmcplugin.addSortMethod(HANDLE, xbmcplugin.SORT_METHOD_TITLE)
        xbmcplugin.addSortMethod(HANDLE, xbmcplugin.SORT_METHOD_VIDEO_YEAR)
        xbmcplugin.addSortMethod(HANDLE, xbmcplugin.SORT_METHOD_VIDEO_RATING)        
    ...
    for index, movie in enumerate(movies):
        ...
        # Kodi - TMDB "tagline" from JSON 
        info_tag.setTagLine(movie.get("tagline"))
        
        # Kodi - TMDB "rating" from JSON 
        info_tag.setRating(round(movie['vote_average'], 1)) if movie.get('vote_average') is not None else info_tag.setRating(None)

        # Kodi - TMDB "runtime" from JSON 
        runtime = movie.get("runtime")
        if runtime:
            info_tag.setDuration(int(runtime) * 60)

        # Kodi - TMDB "year" from JSON            
        info_tag.setYear(int(movie['release_date'][0:4]) if movie['release_date'] != "" else None)            
        
        # Kodi - TMDB "genres" from JSON
...
def get_list(list_type, list_id):
    ...
    """
    list_lang = Addon().getSettingString('tmdb_language')
    list_root = Addon().getSettingString('general_url')
    list_root = list_root if list_root.endswith('/') else list_root + '/'
    
    list_url = list_root + list_lang + '/'
